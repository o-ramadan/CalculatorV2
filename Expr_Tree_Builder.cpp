...
{
	Subtract_Node * subtract = new Subtract_Node();
...
{
	Multiply_Node * multiply = new Multiply_Node();
...
{
	Divide_Node * divide = new Divide_Node();
...
{
	//Connect the binary operator (in the argument) to 2 subtrees in the treeStack_ (by connecting it to their roots)
	//Making them its right and left children (then pop the 2 subtrees off treeStack) 
	//then push the resulting tree onto the treeStack
